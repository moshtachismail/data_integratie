...
def get_data(column_value_pair):
    ...
    # Participant, Birth year, birth month, MAKEN OP BASIS VAN SEX (8507M, 8532F), Sex, 4147507, Ethnicity
    ...
    if column_value_pair.get("Sex") != None:
        if "m" in column_value_pair.get("Sex").lower():
            gender_concept_id = "8507M," + column_value_pair.get("Sex") + ","
        else: 
            gender_concept_id = "8532F" + column_value_pair.get("Sex") + ","
        returnstring += gender_concept_id
    else:
        returnstring += ",,"

    if column_value_pair.get("Ethnicity") != None:
        returnstring += column_value_pair.get("Ethnicity")
